...
function titleFromTarget(target,fallback){
  try{
    ...
    if(food && words.length>1){const emoji=food.split(' ')[0];const base=food.replace(/^[^ ]+\s*/,'');return `${emoji} ${base} en ${words.slice(1).map(w=>w.charAt(0).toUpperCase()+w.slice(1)).join(' ')}`;}
...
function replaceGenericControls(html){
  return html.replace(/<(a|button)\b([^>]*)>([\s\S]*?)<\/\1>/gi,(m,tag,attrs,inner)=>{
    ...
    const label=titleFromTarget(target,'');
    if(!label || generic(label)) return m;
...
walk(OUT,file=>{
  ...
  const before=html;
  const afterShortcuts=removePopularShortcuts(html);
