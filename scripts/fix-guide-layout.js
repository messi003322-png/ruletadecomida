...
@media(max-width:767px){#directorio #rf-final-moment,#directorio #rf-final-meal,#directorio #rf-final-city{gap:7px!important;margin-bottom:24px!important}#directorio .rf-flow-title{margin-top:20px!important}}
</style>`;

const SCRIPT = `<script id="rf-guide-layout-fix-js">
(function(){
 function ready(fn){if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',fn);else fn()}
 function exact(el,t){return el&&el.textContent&&el.textContent.trim()===t}
 ...
 function init(){
  ...
  Array.prototype.forEach.call(root.querySelectorAll('h1,h2,h3,h4,h5,h6,p'),function(el){
   ...
   if(exact(el,'Comida')||exact(el,'Ciudad')||exact(el,'Momento del día')){el.setAttribute('data-rf-hidden-duplicate','1');el.style.display='none'}
  });
  function add(box,n,text){var prev=box.previousElementSibling;if(prev&&prev.classList&&prev.classList.contains('rf-flow-title'))return;box.parentNode.insertBefore(title(n,text),box)}
...
function run(){
 ...
 fs.writeFileSync(INDEX,html,'utf8');return true;
}
