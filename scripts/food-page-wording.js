...
function cleanLegacyLabels(html){
  html=html.replace(/D[oó]nde comer\s+Qu[eé] comer en\s+([^<.!?]+)(?=<|[.!?])/gi,'$1');
  html=html.replace(/Qu[eé] comer en\s+([^<.!?]+)(?=<|[.!?])/gi,'$1');
  html=html.replace(/D[oó]nde comer\s+([^<.!?]+)(?=<|[.!?])/gi,'$1');
  return html;
}
function run(){
  ...
  walk(DIST);
  console.log(`[food-page-wording] OK: ${changed} HTML procesados; etiquetas legacy de comida eliminadas.`);
}
